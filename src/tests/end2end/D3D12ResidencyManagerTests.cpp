...
TEST_F(D3D12ResidencyManagerTests, CreateResidencyWithResource) {
    ComPtr<IResidencyManager> residencyManager;
    ComPtr<IResourceAllocator> resourceAllocator;
    ASSERT_SUCCEEDED(CreateResourceAllocator(CreateBasicAllocatorDesc(), mDevice.Get(),
                                             mAdapter.Get(), &resourceAllocator,
    ...
    // Creating another residency manager pointer also claims ownership.
    EXPECT_EQ(GetRefCount(residencyManager.Get()), 4);

    // Release the allocator (and allocation), relinquishing ownerships of |residencyManager|.
